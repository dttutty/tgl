...
parser.add_argument(
    ...
    choices=(1, 2, 3),
    help=(
        'Logical mailbox/memory staleness to simulate. '
        '1 matches current train_dist.py training semantics '
        '(batch n updates can first affect batch n+2); '
        '2/3 add one/two extra stale macro-batches.'
    ),
)
...
if args.local_rank < args.num_gpus:
    ...
    pending_mailbox_updates = deque()
    extra_mailbox_read_skips = args.stale_macro_batches - 1
    ...
    def _clear_pending_mailbox_updates():
        pending_mailbox_updates.clear()

    def _apply_delayed_mailbox_update(update: DelayedMailboxUpdate) -> None:
        mailbox.update_mailbox(
            ...
            update.edge_feats,
            update.block,
            peer_memory=update.peer_memory,
        )
    ...
    def _advance_pending_mailbox_updates_before_read() -> None:
        if mailbox is None:
            return
        while pending_mailbox_updates and pending_mailbox_updates[0].remaining_read_skips == 0:
            _apply_delayed_mailbox_update(pending_mailbox_updates.popleft())
        if extra_mailbox_read_skips == 0:
            return
        for pending in pending_mailbox_updates:
            if pending.remaining_read_skips > 0:
                pending.remaining_read_skips -= 1

    def _enqueue_delayed_mailbox_update(root_nodes, ts, eid, block) -> None:
        if mailbox is None:
            return
        mem_edge_feats = gather_feature_rows(edge_feats, eid) if edge_feats is not None else None
        pending_mailbox_updates.append(
            DelayedMailboxUpdate(
                nid=model.module.memory_updater.last_updated_nid,
                memory=model.module.memory_updater.last_updated_memory,
                root_nodes=root_nodes.copy(),
                ts=ts.copy(),
                edge_feats=mem_edge_feats,
                block=block,
                memory_ts=model.module.memory_updater.last_updated_ts,
                peer_memory=getattr(model.module, 'last_mail_peer_memory', None),
                remaining_read_skips=extra_mailbox_read_skips,
            )
        )
    while True:
        my_model_state = [None]
        ...
        if my_model_state[0] == -1:
            ...
            if prev_thread is not None:
                ...
                if mailbox is not None:
                    ...
                    _advance_pending_mailbox_updates_before_read()
                ...
                if mailbox is not None:
                    with torch.no_grad():
                        _enqueue_delayed_mailbox_update(
                            prev_thread.get_root(),
                            prev_thread.get_ts(),
                            prev_thread.get_eid(),
                            prev_thread.get_block(),
                        )
                prev_thread = curr_thread
            else:
                _clear_pending_mailbox_updates()
                my_mfgs = [None]
                multi_mfgs = [None] * (args.num_gpus + 1)
                my_root = [None]
                multi_root = [None] * (args.num_gpus + 1)
