...
def run_model(model_name, dataset, config_path, extra_args, gpu):
    cmd = [
    ...
    print(f"\n{'='*60}")
    print(f"[{model_name} / {dataset}] {' '.join(cmd)}")
    print(f"{'='*60}\n")
...
def parse_output(output, model_name, dataset):
    ...
    if avg_match:
        row = {"model": model_name, "dataset": dataset, "epoch": "avg"}
        for j, field in enumerate(FIELDS):
...
def main():
    ...
    for dataset, extra_args in DATASETS.items():
        for model_name, config_path in MODELS.items():
            output = run_model(model_name, dataset, config_path, extra_args, args.gpu)
            all_rows.extend(parse_output(output, model_name, dataset))

    csv_path = os.path.join(REPO_ROOT, "experiment", "timing_results.csv")
    csv_fields = ["model", "dataset", "epoch"] + FIELDS + ["train_loss", "val_ap", "val_auc", "test_ap", "test_auc_mrr"]
