...
public:
    ...
    std::vector<omp_lock_t> ts_ptr_lock;
        ...
        num_history(_num_history), window_duration(_window_duration)
    {
        ...
        ts_ptr_lock.resize(indptr.size() - 1);
        for (auto& lock : ts_ptr_lock) {
            omp_init_lock(&lock);
        }
...
#pragma omp parallel for schedule(static, 1)
    ...
    {
        std::vector<std::vector<NodeIDType>> row_buf(num_threads);
        std::vector<std::vector<NodeIDType>> col_buf(num_threads);
        std::vector<std::vector<EdgeIDType>> eid_buf(num_threads);
        std::vector<std::vector<TimeStampType>> ts_buf(num_threads);
        std::vector<std::vector<TimeStampType>> dts_buf(num_threads);
        std::vector<std::vector<NodeIDType>> nodes_buf(num_threads);
        ...
        for (int i = 0; i < num_history; i++)
        {
            if (!from_root)
            {
                int root_layer = static_cast<int>(ret.size()) - i - num_history - 1;
                // if num_history = 2, ret_size = 5, root_layer = 5 - 0 - 2 - 1 = 2, 5 - 1 - 2 - 1 = 1
                root_nodes = &(ret[root_layer].nodes);
                root_ts = &(ret[root_layer].ts);
            }
            ...
            // if num_history = 2, num_history - 1 - i = 2 - 1 - i = 1 - i: 1, 0
            ret[0].ptr_time += omp_get_wtime() - t_ptr_s;
            ...
            for (int tid = 0; tid < num_threads; ++tid) {
                row_buf[tid].reserve(reserve_capacity);
                col_buf[tid].reserve(reserve_capacity);
                eid_buf[tid].reserve(reserve_capacity);
                ts_buf[tid].reserve(reserve_capacity);
                dts_buf[tid].reserve(reserve_capacity);
                nodes_buf[tid].reserve(reserve_capacity);
            }
