...
RUN_DIR="${RUN_DIR:-$SCRIPT_DIR/seed_sweeps/tgl_jodie_train_py_${DATASET,,}_${STAMP}}"
mkdir -p "$RUN_DIR"

RESULTS_TSV="$RUN_DIR/results.tsv"
SUMMARY_TXT="$RUN_DIR/summary.txt"
TMP_CONFIG="$(mktemp "$RUN_DIR/JODIE_XXXXXX.yml")"
...
echo "Running TGL JODIE seed sweep via train.py on GPU ${GPU_ID}"
...
for seed in "${SEEDS[@]}"; do
  ...
  echo
  echo "=== [TGL JODIE train.py] seed=${seed} ==="
  ...
  echo "[TGL JODIE train.py] seed=${seed} final_test_ap=${test_ap}"
done
